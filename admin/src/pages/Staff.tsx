  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const personel: LokalnyUzytkownik[] = dane
          .filter(
            (u) =>
              (u.role_name &&
                ["admin", "moderator"]
                  .includes(String(u.role_name).toLowerCase()))
          )
          .map((u) => {
            const statusValue: "aktywny" | "nieaktywny" | "oczekujący" =
              typeof u.is_active === "boolean"
                ...
                : "aktywny";
            const roleLabel =
              u.role_name && String(u.role_name).toLowerCase() === "admin"
                ? "Administrator"
                : u.role_name &&
                  String(u.role_name).toLowerCase() === "moderator"
                ? "Moderator"
                : "Personel";
            return {
              id: u.id,
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="page-header">Personel</h1>
          <p className="page-description">Zarządzanie kontami administratorów i moderatorów</p>
        </div>
      </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              {/* <TableHead className="text-center">Wydarzenia</TableHead> */}
          ...
          <TableBody>
            {ladowanie ? (
              <TableRow>
                <TableCell colSpan={6}>
                  <div className="py-6 text-center">Ładowanie personelu...</div>
                </TableCell>
              </TableRow>
            ) : przefiltrowani.length === 0 ? (
              <TableRow>
                <TableCell colSpan={6}>
                  <div className="py-6 text-center">Brak wyników</div>
                </TableCell>
              </TableRow>
            ) : (
              przefiltrowani.map((u) => (
                <TableRow key={u.id} className="cursor-pointer">
                  ...
                  {/* <TableCell className="text-center">{u.events}</TableCell> */}
      ...
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        <span>Wyświetlono {przefiltrowani.length} z {uzytkownicy.length} członków personelu</span>
